                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped" id="save-stage" style="width:100%;">
                                        ...
                                        <tbody>
                                            @foreach ($data_direksi as $direksi)
                                                <tr class="text-center">
                                                    <td class="text-center">{{ $loop->iteration }}</td>
                                                    <td>{{ date('d F Y', strtotime($direksi->tanggal)) }}</td>
                                                    <td>{{ $direksi->judul }}</td>
                                                    <td class="text-center">
                                                        {{-- <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                                            action="{{ route('direksi.destroy', $direksi->id) }}" method="POST"> --}}
                                                            <button type="button" class="show btn btn-sm btn-outline-info"
                                                                data-id="{{ $direksi->id }}"
                                                                data-judul="{{ $direksi->judul }}"
                                                                data-isi="{{ $direksi->isi }}">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('direksi.store') }}" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
