                                        ...
                                        <tbody>
                                            @foreach ($data_kredit as $kredit)
                                                <tr>
                                                    ...
                                                    <td>{{ date('d F Y', strtotime($kredit->tanggal)) }}</td>
                                                    <td>{{ $kredit->nama }}</td>
                                                    <td>{{ $kredit->produk_layanan}}</td>
                                                    <td class="text-center">
                                                        <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                                            action="{{ route('buka.kredit.destroy', $kredit->id) }}"
                                                            method="POST">
                                                            {{-- <button type="button"
                                                                class="show btn btn-sm btn-outline-info"
                                                                data-id="{{ $kredit->id }}"
                                                                data-nama="{{ $kredit->nama }}"
                                                                data-produk_layanan="{{ $kredit->produk_layanan }}">
                                                                <i data-feather="info"></i> Detail
                                                            </button> --}}
                                                            <a href="{{ route('buka.kredit.detail', $kredit->id) }}"
